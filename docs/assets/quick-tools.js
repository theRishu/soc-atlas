(function() {
  /* --- SOCAtlas Success Engine (v4.0 - Progress Only Mode) --- */
  ...
  // Dashboard showing overall percentage (HOME ONLY)
  ...
  // Quick Revision Points (Auto-Mastery Removed, Checkboxes Only)
  function initQuickPath() {
    const isQuick = window.location.pathname.includes('/quick/'); if (!isQuick) return;
    const tables = document.querySelectorAll('.md-content__inner table'); const stored = getStorage(QUICK_PATH_KEY);
    tables.forEach((table, tIdx) => {
      if (table.querySelector('.quick-point-check')) return;
      ...
      table.querySelectorAll('tbody tr').forEach((row, rIdx) => {
        ...
        cb.onchange = () => { const fr = getStorage(QUICK_PATH_KEY); if (cb.checked) { fr[pid] = true; row.classList.add('point-mastered'); } else { delete fr[pid]; row.classList.remove('point-mastered'); } setStorage(QUICK_PATH_KEY, fr); };
        td.appendChild(cb); row.insertBefore(td, row.firstChild);
      });
    });
  }

  // Automatic "Silent" certification (No panels, just marks lesson done when user reaches bottom)
  function initSilentMastery() {
    const pathId = getPathId(window.location.pathname); if (pathId === 'home' || pathId === 'none' || window.location.pathname.includes('/quick/')) return;
    window.onscroll = () => {
        if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 50) {
            const fr = getStorage(GUIDED_PATH_KEY); if (!fr[pathId]) { fr[pathId] = true; setStorage(GUIDED_PATH_KEY, fr); updateSidebar(); }
        }
    };
  }

  function start() { try { initDashboard(); initQuickPath(); initSilentMastery(); updateSidebar(); } catch (e) {} }
